...
void ecan_init()
{
    ...
    BRGCON1bits.SJW0 = 0;
    //maybe look at if does not work
    BRGCON1bits.BRP5 = 0;       // Baud Rate Pre-scaler bits
    BRGCON1bits.BRP4 = 0;       // TQ = (2 * 1)/ 16 = 125 ns
    BRGCON1bits.BRP3 = 0;
    BRGCON1bits.BRP2 = 0;
    BRGCON1bits.BRP1 = 0;
    BRGCON1bits.BRP0 = 0;
    ...
    short sid0 = 0x00A;             // SID for Levitation
    short mask0 = 0x7FF;
    ...
    RXFCON0bits.RXF0EN = 1;         // disable filter with 0
...
void ecan_receive_rxb0(Message* mess)
{
    ...
    //while(!(RXB0CONbits.RXB0FUL || PIR3bits.RXB0IF));    // Wait until message is ready
...
{
    /* Make sure that there is a message pending in RXB0. */
    //while(!(RXB1CONbits.RXB1FUL || PIR3bits.RXB1IF));    // Wait until message is ready
...
/* Check if receive buffer 1 has received a message */
